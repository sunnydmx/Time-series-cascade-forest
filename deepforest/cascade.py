...
'''
Estimator，中文意思就是估计器。
在sklearn中，所有的机器学习模型都是Estimator，即他们都继承自Estimator，
当然这个过程也可能涉及多重继承。作为机器学习模型，其一大共通之处就在于会涉及到一些参数。
关于参数的处理就是在BaseEstimator中实现的。BaseEstimator有两个重要的方法，
一个是get_params，另一个是set_params，很明显，一个是用来获取参数，一个是用来设置参数。

class sklearn.base.ClassifierMixin：为所有的classifiers提供Mixin class
__init__()	初始化方法
score(X, y, sample_weight=None)	 返回给定测试数据和标签的平均度量值
...
def _build_classifier_predictor(
    ...
    """Build the predictor concatenated to the deep forest.构建连接到深林的预测变量。"""
...
class BaseCascadeForest(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        X_train_ = self.buffer_.cache_data(0, X, is_training_data=True)
        # print("X_train_")
        # print(X_train_)
        # =====================================================================
        # Training Stage
...
class CascadeForestClassifier(BaseCascadeForest, ClassifierMixin):
    def __init__(
        ...
        super().__init__(shape_1X=shape_1X, window=window, stride=stride, n_bins=n_bins, bin_subsample=bin_subsample, bin_type=bin_type, max_layers=max_layers,
                         criterion=criterion, n_estimators=n_estimators, n_trees=n_trees, max_depth=max_depth,
                         min_samples_leaf=min_samples_leaf, use_predictor=use_predictor, predictor=predictor,
                         predictor_kwargs=predictor_kwargs, backend=backend, n_tolerant_rounds=n_tolerant_rounds,
                         delta=delta, partial_mode=partial_mode, n_jobs=n_jobs, random_state=random_state,
                         verbose=verbose)
    ...
    def predict_proba(self, X):
        ...
        proba : :obj:`numpy.ndarray` of shape (n_samples, n_classes)
            The class probabilities of the input samples.输入样本的类概率。
        """
        ...
        # X_test = self._bin_data(binner_, mgs_X, is_training_data=False)
...
class CascadeForestRegressor(BaseCascadeForest, RegressorMixin):
    def __init__(
        ...
        super().__init__(n_bins=n_bins, bin_subsample=bin_subsample, bin_type=bin_type, max_layers=max_layers,
                         criterion=criterion, n_estimators=n_estimators, n_trees=n_trees, max_depth=max_depth,
                         min_samples_leaf=min_samples_leaf, use_predictor=use_predictor, predictor=predictor,
                         predictor_kwargs=predictor_kwargs, backend=backend, n_tolerant_rounds=n_tolerant_rounds,
                         delta=delta, partial_mode=partial_mode, n_jobs=n_jobs, random_state=random_state,
