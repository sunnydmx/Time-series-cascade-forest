...
def _find_binning_thresholds_per_feature(
    ...
    # Too few distinct values 非重复值太少
    if len(distinct_values) <= n_bins:
        midpoints = distinct_values[:-1] + distinct_values[1:]
        midpoints *= 0.5
    else:
        # Equal interval in terms of percentile 百分位数的相等间隔
        ...
        # Equal interval in terms of value 值相等间隔
...
class Binner(TransformerMixin, BaseEstimator):
    ...
    def fit(self, X):
        ...
        # print("_binner.py 139")
        # print(self.bin_thresholds_)  # 128*29
        self.n_bins_non_missing_ = np.array(
            [thresholds.shape[0] + 1 for thresholds in self.bin_thresholds_],
            dtype=np.uint32,
        )

        # print("_binner.py 146")
        # print(self.n_bins_non_missing_)  # 1*128
